                    ...
                    <form action="{{ route('matriculas.update', $matricula) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" id="data_matricula" name="data_matricula" value="{{ old('data_matricula', $matricula->data_matricula->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" id="valor_mensalidade" name="valor_mensalidade" value="{{ old('valor_mensalidade', $matricula->valor_mensalidade) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div>
                                <label for="numero_parcelas" class="block text-sm font-medium text-gray-700">Número de Parcelas *</label>
                                <input type="number" id="numero_parcelas" name="numero_parcelas" value="{{ old('numero_parcelas', $matricula->numero_parcelas) }}" min="1" max="24" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div>
                                <label for="data_inicio" class="block text-sm font-medium text-gray-700">Data de Início *</label>
                                <input type="date" id="data_inicio" name="data_inicio" value="{{ old('data_inicio', $matricula->data_inicio->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div>
                                <label for="data_fim" class="block text-sm font-medium text-gray-700">Data de Fim *</label>
                                <input type="date" id="data_fim" name="data_fim" value="{{ old('data_fim', $matricula->data_fim->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                        </div>

                        <div class="mt-6">
                            <label for="observacoes" class="block text-sm font-medium text-gray-700">Observações</label>
                            <textarea id="observacoes" name="observacoes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('observacoes', $matricula->observacoes) }}</textarea>
                        </div>
