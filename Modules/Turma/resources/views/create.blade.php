...
@section('content_header')
    <h1><i class="fas fa-users"></i> Nova Turma</h1>
@stop

@section('content')
    <div class="card">
        <form method="POST" action="{{ route('turmas.store') }}" id="turma-form">
            @csrf
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12 form-group">
                        <label for="nome">Nome da Turma <span class="text-danger">*</span></label>
                        <input id="nome" class="form-control @error('nome') is-invalid @enderror" type="text" name="nome" value="{{ old('nome') }}" required placeholder="Ex: 3º Ano A">
                        @error('nome')<span class="invalid-feedback">{{ $message }}</span>@enderror
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="codigo">Código <span class="text-danger">*</span></label>
                        <input id="codigo" class="form-control @error('codigo') is-invalid @enderror" type="text" name="codigo" value="{{ old('codigo') }}" required placeholder="Ex: 3A-2026">
                        @error('codigo')<span class="invalid-feedback">{{ $message }}</span>@enderror
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="ano">Ano Letivo <span class="text-danger">*</span></label>
                        <input id="ano" class="form-control @error('ano') is-invalid @enderror" type="number" name="ano" value="{{ old('ano', date('Y')) }}" required min="2000" max="2100">
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        <input id="vagas_total" class="form-control @error('vagas_total') is-invalid @enderror" type="number" name="vagas_total" value="{{ old('vagas_total', 30) }}" required min="1">
                        @error('vagas_total')<span class="invalid-feedback">{{ $message }}</span>@enderror
                    </div>
                    <div class="col-md-12 form-group">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="ativo" name="ativo" value="1" {{ old('ativo', true) ? 'checked' : '' }}>
